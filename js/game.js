var Game = {
	...
	_move		: function(id) {
		...
		if (!(Math.abs(object.vector.w) > this._round || Math.abs(object.vector.h) > this._round || Math.abs(object.accel.w) > this._round || Math.abs(object.accel.h) > this._round)) {
			object.stat	= 0;
			return;
		}
		...
		var wCondition	= (vector.w < 0 && accel.w <= 0) ? (w >= this.range.min.width) : (w <= object.limit.w);
		var hCondition	= (vector.h < 0 && accel.h <= 0) ? (h >= this.range.min.height) : (h <= object.limit.h);
		if (wCondition && hCondition) {
			w		+= Number(vector.w);
			h		+= Number(vector.h);
			vector.w	+= Number(accel.w);
			vector.h	+= Number(accel.h);
			if (object.isBound && ((vector.w < 0 && accel.w <= 0 && w <= this.range.min.width) || (vector.w > 0 && w >= object.limit.w))) {
				offset		= (((vector.w < 0) ? this.range.min.width : object.limit.w) - w);
				w		= (offset / 2) + ((vector.w < 0) ? this.range.min.width : object.limit.w);
				vector.w	*= -1;
				if (accel.w) vector.w *= accel.w;
			}
			if (object.isBound && ((vector.h < 0 && accel.h <= 0 && h <= this.range.min.height) || (vector.h > 0 && h >= object.limit.h))) {
				offset		= (((vector.h < 0) ? this.range.min.height : object.limit.h) - h);
				h		= (offset / 2) + ((vector.h < 0) ? this.range.min.height : object.limit.h);
				vector.h	*= -1;
				if (accel.h) vector.h *= accel.h;
			}
			if (w < this.range.min.width) w = this.range.min.width;
			if (w > object.limit.w) w = object.limit.w;
			if (!object.isBound && (w === this.range.min.width || w === object.limit.w)) vector.w = 0;
			if (h < this.range.min.height) h = this.range.min.height;
			if (h > object.limit.h) h = object.limit.h;
			if (!object.isBound && (h === this.range.min.height || h === object.limit.h)) vector.h = 0;
	...
	setObject	: function(id) {
		...
		object.limit.w		= this.range.max.width - object.w;
		object.limit.h		= this.range.max.height - object.h;
	...
	move		: function(id, wVector, hVector, wAccel, hAccel, isBound) {
		...
		if (this._rmUnit(object.element.style.left) < this.range.min.w) object.element.style.left = this.range.min.w;
		if (this._rmUnit(object.element.style.top) > object.limit.h) object.element.style.top = object.limit.h;
		if (this._rmUnit(object.element.style.top) < this.range.min.h) object.element.style.top = this.range.min.h;
		if (object.stat == 0) {
			object.stat	= 1;
			this._move(id);
		}
